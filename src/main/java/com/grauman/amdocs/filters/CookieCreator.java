    ...
    public static Cookie createUserCookie(int id, String email, List<Role> roles, List<Permission> permissions){
        ...
        for(int i = 0; i < roles.size(); i++){
            if (i == roles.size()-1){
                ...
                values.append(roles.get(i).getName());
            }else{
                values.append(roles.get(i).getName()).append(",");
            }
        }
        ...
        for(int i = 0; i < permissions.size(); i++){
            if (i == permissions.size()-1){
                values.append(permissions.get(i).getName());
            }else{
                values.append(permissions.get(i).getName()).append(",");
            }
        }
        values.append("]:");
